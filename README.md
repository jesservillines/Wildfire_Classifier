...
### Project 5: A project focused on prediction of fire risk based on meteorological data.
...
This project focused on the following 11 States in the US: 
|States| | 
|---|---|
|Arizona|Montana |
...
This project uses two sources of data which were studied through EDA and then combined into a single data frame which informed the modeling phase:
...
This project also took a deep dive into NOAA wind data but discovered that the combined datasets were far too large to add to our existing dataframe.  Wind direction is a great weather predictor, and because wind speed can feed fires, we believe that adding wind data, such as wind speed, gusts and potentially wind direction would have added significant value to our models.

Another interesting data set encountered was foliage data from Google Earth Engine. This required setting up an account with Google and being accepted to use their engine, and then exploring data using Javascript. It became too cumbersome for the scope of this project.
...
To improve our models, we employed the modeling technique of bootstrapping which gave us a more normal distribution of wildfire classes. This way, the models were able to capture the larger fires. It greatly improved recall which is ultimately the target this project wanted to pursue, as this helped predict larger and more destructive wildfires. 

The second modeling breakthrough was harnessing geospatial data through KMeans clustering of longitude and latitudinal data. We then One-Hot-Encoded it which created a sparse matrix that was the most important predictive element of the model. This is because terrain features matter immensely when determining the potential size of a wildfire. (See confusion matrix below)

The third major breakthrough was the trailing averages as noted in the summary above. Adding that data essentially doubled our recall scores for medium sized fires, which improved our overall model recall. Next, since our Neural Network is a blackbox model, and as such this was able to glean as much insight into features. For feature importance this project utilizes Random Forest Classification to compliment insights from our Neural Network model by providing top features and weights.
...
1. Wildfires are extremely complex phenomena. While the NOAA data offered a set of independent variables which fairly comprehensively described weather history, other important modeling factors which also affect final fire size, such as wind or terrain features (e.g. land cover or incline) were not included.

2. Switching the target variable from a continuous one (fire size, in acres) to a multi-class problem improved the score from an R2 of under 10% to accuracy of over 60%. Further, re-defining the problem as a binary classification of "large" vs "small" fires drove accuracy up to 62-77%, depending on the threshold chose to delineate between the two classes.

3. Because the goal was to improve preparedness and help contain damage from wildfires without putting efforts into preventing fires which weren't likely to spread, the ultimate focus was on increasing the recall of our model, and moreover - to increase its recall with regards to large fires.
...
5. Despite being computationally expensive, the project was able to create a model that gave a significant increase in our recall score. This model can confidently predict very large fires and some mid range fires with the current model.
